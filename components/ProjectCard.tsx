  ...
  if (featured) {
    return (
      <article
        ...
        className="group relative rounded-2xl overflow-hidden bg-mist aspect-[16/6]"
      >
        <div className="absolute inset-0 gradient-bg opacity-20 group-hover:opacity-30 transition-opacity duration-300" />

        <Link
          href={`/projects/${project.slug}`}
          className="absolute inset-0 flex flex-col justify-end p-8 md:p-12 bg-gradient-to-t from-deep/70 to-transparent"
        >
